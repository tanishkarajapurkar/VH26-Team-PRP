import { Category, Product, FlashSale, Review } from '../types';
...
export const LOCAL_FLASH_SALES: FlashSale[] = [
  {
    id: 'sale_prime_surge',
    ...
    status: 'active',
    products: LOCAL_PRODUCTS.filter(p => p.is_flash_sale).map(p => ({
      ...p,
      sale_price: p.flash_sale_price || p.price,
      stock_limit: p.stock + 20,
      sold_count: Math.round((p.stock + 20) * ((p.flash_sale_claimed_percent || 75) / 100))
    }))
  }
];
