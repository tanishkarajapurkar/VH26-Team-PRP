...
export function markUserActivity(sessionId) {
...
trafficRoutes.get('/activity', (_req, res) => {
    const now = Date.now();
...
trafficRoutes.post('/events', async (req, res) => {
    try {
        const { endpoint, method, status_code, response_time, source, scenario, session_id, product_id } = req.body;
        if (!endpoint || !method) {
...
trafficRoutes.get('/stats', (_req, res) => {
